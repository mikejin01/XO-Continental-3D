    ...
    <AnimatedSection className="bg-black px-5 py-28 sm:px-8 lg:px-10" delay={0.05}>
      <div className="mx-auto max-w-[1440px]" id="services">
        <div className="mb-7 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <h2 className="text-[clamp(38px,4.6vw,72px)] font-bold leading-[0.95] tracking-[-0.04em] text-transparent bg-gradient-to-r from-[#96D1B0] via-[#F5F5F8] to-[#96D1B0] bg-clip-text">
            Our Services
          </h2>
          <a
            href="#contact"
            className="inline-flex w-fit items-center gap-2 rounded-full border border-[#3EC092]/40 px-6 py-3.5 text-sm text-white shadow-[0_0_24px_rgba(62,192,146,0.18)] transition hover:-translate-y-0.5"
          >
            Learn more ↗
          </a>
        </div>
          ...
          {serviceCards.map((card, index) => (
            ...
            >
              <div className="absolute inset-0">
                {/* Background video — always visible, starts grayscale, becomes color on hover */}
                <video
              ...
              <div className="relative z-20 flex h-full flex-col justify-end p-7">
                <div className="flex items-end justify-between gap-4">
                  <h3 className="text-[clamp(28px,2.5vw,48px)] font-semibold leading-[1.05] tracking-[-0.03em] text-white/75 transition duration-300 group-hover:-translate-y-14 group-hover:bg-gradient-to-r group-hover:from-[#96D1B0] group-hover:via-[#F5F5F8] group-hover:to-[#96D1B0] group-hover:bg-clip-text group-hover:text-transparent">
                      {card.title}
                  </h3>
